...
public final class Field {
    ...
    private static final Map<Integer, Pose3d> APRIL_TAG_ORIGINALS = Map.of(
        1,
    ...
    public static final Pose2d reflectPosition(final Pose2d pose) {
        return new Pose2d(FIELD_LENGTH - pose.getTranslation().getX(),
                          FIELD_WIDTH - pose.getTranslation().getY(),
    ...
    // a load zone
    public static Map<Integer, Boolean> OUR_TAG_IDS =
    ...
    private static final Map<Integer, Pose3d> reflectAprilTags() {
        ...
        for (final Map.Entry<Integer, Pose3d> aprilTag :
             APRIL_TAG_ORIGINALS.entrySet()) {
    ...
    public static List<AprilTag> getAprilTagsList() {
        return getAprilTagsMap()
            .entrySet()
            .stream()
            .map(entry -> new AprilTag(entry.getKey(), entry.getValue()))
            .toList();
    }

    public static AprilTagFieldLayout getCurrentFieldLayout() {
        return new AprilTagFieldLayout(getAprilTagsList(), FIELD_LENGTH,
                                       FIELD_WIDTH);
    }
