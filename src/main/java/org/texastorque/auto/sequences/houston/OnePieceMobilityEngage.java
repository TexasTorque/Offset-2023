...
public final class OnePieceMobilityEngage extends TorqueSequence implements Subsystems {
    public OnePieceMobilityEngage() {
        ...
        addBlock(new DriveUntil(1, () -> TorqueNavXGyro.getInstance().getPitch() < -15));

        addBlock(new DriveUntil(.5, () -> TorqueNavXGyro.getInstance().getPitch() > 15));

        addBlock(new DriveUntil(.5, () -> TorqueNavXGyro.getInstance().getPitch() > -1 && TorqueNavXGyro.getInstance().getPitch() < 1));

        addBlock(new FollowEventPath("devastating-davey"));
